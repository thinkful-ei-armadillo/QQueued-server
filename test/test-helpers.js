...
const testHelpers = {
  ...
  createFixtures() {
    ...
    const testQueue = this.createQueue();
    const testPointers = this.createPointers();
    const testStudentData = this.createStudentData();
    return { testUsers, testQueue, testPointers, testStudentData };
  },
  ...
  seedStudentData(db, user, queue, studentData) {
    return db.transaction(async trx => {
      await this.seedUser(trx, user)
      await this.seedQueue(trx, queue);
      await trx.into('studentData').insert(studentData);
    })
  },
